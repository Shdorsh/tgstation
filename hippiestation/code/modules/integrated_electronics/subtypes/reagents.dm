...
/obj/item/integrated_circuit/reagent/drain/do_work()
	if(get_pin_data(IC_OUTPUT, 1, reagents.total_volume))
		...
		//Doing one after another to not get a runtime error in case where one of these is missing
		if(!drainedchems)
			push_data()
			activate_pin(2)
			return
		if(!drainedchems.reagents)
			push_data()
			activate_pin(2)
			return
		if(drainedchems.reagents.total_volume == 0)
			...
			activate_pin(2)
			return
		drainedchems.reagents.trans_to(src, 30, 0.8)
		if(drainedchems.reagents.total_volume == 0)
			del(drainedchems)
	push_data()
	activate_pin(2)
